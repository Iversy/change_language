...
def press_release(key:str, delay:float):
    k.press(key)
    time.sleep(delay)
    k.release(key)
...
def _change_letter(letter:str, diff:int, up=False) -> str:
    ltr_order = ord(letter)
    if ltr_order < ord('A') or ord('z') < ltr_order < ord('А') or ord('я') < ltr_order: 
        return letter
    return chr(ltr_order + diff) if up else chr(ltr_order - diff)

# ru-en only
def change_text():
    pyperclip.copy("")
    time.sleep(0.1)
    press_release("ctrl+a", 0.4)
    press_release("ctrl+c", 0.4)
    time.sleep(0.1)
...
k.add_hotkey("ctrl+insert", change_text, timeout=1)
k.wait()
